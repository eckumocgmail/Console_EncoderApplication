...
{        
    ...
    public KeyValuePair<IEnumerable<int>, TextTreeNode> Build( )
    {
        Encodes.Clear();
        var rating = new List<TextTreeNode>();
         
        List<int> metadata = new List<int> { Statistics.Count() };
        foreach (var kv in Statistics){
            metadata.Add(kv.Key);
            metadata.Add(kv.Value);
            rating.Add(new TextTreeNode(new List<byte>() { kv.Key },kv.Value));
        }
        ...
        while (rating.Count() > 1)
        {
            ...
            TextTreeNode left = rating[0];
            TextTreeNode right = rating[1];
            rating.RemoveAt(1);
            rating.RemoveAt(0);
            rating.Add(new TextTreeNode(chars, count)
            {
                Left = left,
                Right = right
            });
        ...
        WriteLine("Tree");
        rating[0].Trace();
        return new KeyValuePair<IEnumerable<int>, TextTreeNode>(metadata, rating[0]);
    }
    public void LogInfo()        
...
public class CharDecoder
{
    ...
    private void SetRoot(TextTreeNode root)
    {
        this._root = root;
    }
    public IEnumerable<byte> Decode( )
    {
        TextTreeNode p = _root;
...
{
    private TextTreeNode _root;
    public CharEncoder(TextTreeNode root)
    {
        this._root = root;
    }

    public IEnumerable<bool> EncodeData(string text)
    {
    ...
    public List<bool> EncodeMetadata(IEnumerable<int> indexes)
    {
        var result = new List<bool>();
        var bitConverter = new BitConverter();
